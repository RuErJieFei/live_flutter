...
class ScheduleDetailSearchView extends GetView<SchedulePageController> {
  final int scheduleId;
  const ScheduleDetailSearchView(
      this.scheduleId, {
        Key? key,
      }) : super(key: key);

 Future getScheduleById() async {
   var param =  {"id":scheduleId};
   var res  = await request.get("/schedule/searchScheduleById",params:param);
   print(res);
   return res;
 }
  @override
      ...
      builder: (BuildContext context, AsyncSnapshot snapshot) {
        ...
        if (snapshot.connectionState == ConnectionState.done) {
          if (snapshot.hasError) {
            ...
            // 请求成功，显示数据
            Data schedule = Data.fromJson(snapshot.data);
                  ...
                  children: [
                    Row(
                      children: [
                        ...
                        SizedBox(width: 60.w,),
                        Text("${schedule.description}", style: TextStyle(fontSize: 18.sp)),
                      ],
                    ),
                    SizedBox(height: 8.h,),
                    SykDivider(context, 2),
                  ...
                  children: [
                    Row(
                      children: [
                        ...
                        SizedBox(width: 60.w,),
                        Text("${schedule.alertTime}分钟前", style: TextStyle(fontSize: 18.sp)),
                      ],
                    ),
